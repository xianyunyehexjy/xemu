...
static bool pgraph_process_report_queue_entry(NV2AState *d, QueryReport *r)
{
    PGRAPHState *pg = &d->pgraph;
...
void pgraph_process_report_queue(NV2AState *d)
{
    PGRAPHState *pg = &d->pgraph;
    QueryReport *r, *next;

    QSIMPLEQ_FOREACH_SAFE(r, &pg->report_queue, entry, next) {
        bool handled = pgraph_process_report_queue_entry(d, r);
        if (!handled) {
            break;
        }
